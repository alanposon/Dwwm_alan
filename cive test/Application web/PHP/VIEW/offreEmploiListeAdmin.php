...
foreach ($listeOffreEmploi as $offre) {
    ...
    if ($lvl > 2) {
        ...
        $affichage .= '<a href="index.php?action=offreEmploiFormAdmin&id=' . $offre->getIdOffreEmploi() . '&act=modif">   <div class="bouton"> modifier </div></a>
                <a href="index.php?action=offreEmploiFormAdmin&id=' . $offre->getIdOffreEmploi() . '&act=suppr">   <div class="bouton"> supprimer</div></a>
            </div>';
    } else {
       $affichage .= '<a  href="index.php?action=postulerForm"> <div class="bouton">Postuler</div></a></div>  
        ';
    };
}
