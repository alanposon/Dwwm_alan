...
if(!$result["is_valid"])
{
    ...
    <script type="text/javascript">
        function SFValidateLicense(e)
        {
            e.preventDefault();
            debugger;
            RedNaoLicensingManagerVar.ActivateLicense('Please submit your license here');
        }
    </script>

    <div style="background-color:white;padding:10px; border: 1px solid #dfdfdf;margin-top: 10px;">
        <h1>Entries screen is only available in the full version</h1>
        <ul style="margin-left: 5px;">
            <li>* View all the submitted information in one easy to use page</li>
            <li>* Search entries by date</li>
            <li>* Delete not wanted entries</li>
            <li>* Edit each entry</li>
            <li>* Export to csv</li>
        </ul>

        <a target="_blank" href="https://smartforms.rednao.com/getit/">Get the full version here</a>
        <a onclick="SFValidateLicense(event)" style="display: block;margin-top: 5px;" href="#">I already have a license</a>
    </div>
...
<div id="smartDonationRadio" class="smartFormsSlider" style="margin-bottom: 20px;">
    ...
    <select id="cbForm">
        <?php
        global $wpdb;
        $results=$wpdb->get_results("select form_id,form_name from ".SMART_FORMS_TABLE_NAME." order by form_name");
