...
else //On est dans le cas traitement
{
    ...
    $temps = time();
    $pseudo=$_POST['pseudo'];
    $pass = md5($_POST['mot_de_passe']); // on hache le password avec md5
    $confirm = md5($_POST['confirm']);
    $role = 1;
    //Vérification du pseudo
    $utilisateur  = PersonnageManager::get($pseudo);
    if ($utilisateur->getIdPerso()!=null)
    {
    ...
    {
    	$nouvelUtilisateur = new Personnage(['pseudo'=>$_POST['pseudo'],'mot_de_passe'=>md5($_POST['mot_de_passe']),'role'=>$role]);
        PersonnageManager::add($nouvelUtilisateur); // On crée l'utilisateur dans la base
        $nouvelUtilisateur = PersonnageManager::get($_POST['pseudo']); //pour récupérer l'ID
        ...
        $_SESSION['idPerso'] = $nouvelUtilisateur->getIdPerso() ;
