...
{
    $upload_form_field='';
    if(isset($_POST['upload_form_field']))
        $upload_form_field=$_POST['upload_form_field'];
    if(!wp_verify_nonce($upload_form_field,'upload_form'))
    {
...
if($action!=="edit")
{
	...
	echo sprintf(' <a href="?page=%s&action=%s" class="btn btn-default btn-success" ><span class="glyphicon glyphicon-plus" ></span>Add New</a>',$page,'add');
    ?>

    <form style="display: inline; margin-left:10px;" id="sfFileUploadForm" method="post" enctype="multipart/form-data" target="_self" action="?page=<?php echo $page ?>&action=upload">
        <?php wp_nonce_field( 'upload_form', 'upload_form_field' );  ?>
