...
$affichage .= ' <div class="fondOrange"><h2>Chantier</h2> </div>
...
if ($lvl > 2) {
    ...
    $affichage .= '<a class="bouton" href="index.php?action=chantierForm&act=ajout">Ajoutez un chantier</a>  
         ';
} else {
    $affichage .= '</div>'; 
}
...
foreach ($listeChantier as $chan) {
    ...
    if ($lvl > 2) {
        ...
        $affichage .= '<a href="index.php?action=chantierForm&id=' . $chan->getIdChantier() . '&act=modif">   <div class="bouton"> modifier </div></a>
                <a href="index.php?action=chantierForm&id=' . $chan->getIdChantier() . '&act=suppr">   <div class="bouton"> supprimer</div></a></div>';
         
    } else {
       $affichage .= '</div> <a href="index.php?action=chantierForm"> <div class="bouton">chantier</div></a>';
    };
