   <div class="bas">

       <form method="post" action="">



           <h2>Inscription :</h2>
           <div class="formIdent">
               <div class="formuInsc">


                   <p><label for="nom">Nom </label>
                       <input type="text" id="nom" name="nom" value="" /></p>

                   <p><label for="prenom">Prenom </label>
                       <input type="text" id="prenom" name="prenom" value="" /></p>

                   <p><label for="mail">Mail </label>
                       <input type="email" id="mail" name="mail" value="" placeholder="exemple@mail.com" /></p>

                   <p><label for="matricule">Matricule </label>
                       <input type="number" id="matricule" name="matricule" value=" " /></p>

                   <input type="submit" value="Matricule disponible" />


                   <p><label for="motDePasse">Mot de passe</label>
                       <input type="password" id="motDePasse" name="motDePasse" value=" " /></p>

                   <p><label for="confirm">Confirmation du mot de passe</label>
                       <input type="password" id="confirm" name="confirm" value="" /></p>

                   <p><label for="posteEntreprise">Poste en entreprise </label>
                       <input type="number" id="posteEntreprise" name="posteEntreprise" value="" placeholder="1 visiteur ou 2 employé" /></p>





                   <input type="submit" name="inscriptionForm" value="Valider" />
                   <input type="reset" value="Annuler" />
               </div>

       </form>

       <div class="idenCIVEIns">
           <img src="IMAGE/logoCive.png">
           <p> <u>Nos coordonnées :</u>
               <br>
               <div class="coord">
                   <br><strong>CIVE</strong>
                   <br> 2 rue de l'industrie
                   <br> 59820 Gravelines
                   <br> France
                   <br> Tél. 03 28 66 06 49
                   <br> Mail. cive@orange.fr
               </div>
           </p>
       </div>
   </div>
   </div>